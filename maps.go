// <em>マップ</em>は Go に組み込みの[連想データ型](http://en.wikipedia.org/wiki/Associative_array)である。
// これは他の言語では<em>ハッシュ</em>や<em>辞書</em>と呼ばれることがあるものだ。
...
func main() {
	...
	// ここでは、値はいらないので、<em>ブランク識別子</em> `_` を使って、無視している。
	_, prs := m["k2"]
	fmt.Println("prs:", prs)
